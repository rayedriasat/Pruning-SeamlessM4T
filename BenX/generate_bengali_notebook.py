"""
generate_bengali_notebook.py
============================
Run this script to produce:
  bengali_seamless_notebook.ipynb

Changes from v2 (Multilingual full-final):
  • Bengali-centric: Ben↔X and X↔Ben (Ben, Eng, Hindi, Arabic). Chinese removed.
  • Speech Encoder: 24 → 8 layers (prune 16, keep 2/3). Metric: text-BLEU primary, text-ChrF fallback.
  • Text Decoder:   24 → 8 layers (prune 16, keep 2/3). Metric: text-BLEU primary, text-ChrF fallback.
  • T2U:            enc 6→4, dec 6→4 (prune 2 each). Metric: ASR-BLEU primary, ASR-ChrF fallback.
  • Vocab pruning: 4 languages (eng, ben, hin, arb).
  • Full training (no LoRA) — all non-frozen components unfrozen.
  • VRAM-efficient: selective gradient checkpointing, AMP fp16, never OOM.
  • Benchmarks report both BLEU and ChrF (text + ASR).
"""
...
def code(src,outputs=None): return {"cell_type":"code","execution_count":None,"metadata":{},"outputs":outputs or [],"source":src if isinstance(src,list) else [src]}
...
def add(cell): NB["cells"].append(cell)
...
# ══════════════════════════════════════════════════════════════════════════════
# CELL 1 – Setup / paths / platform detection
# ══════════════════════════════════════════════════════════════════════════════
add(md("## ⚙️ Setup — run ALL at the start of EVERY Kaggle session"))
...
# CELL 5 – rclone helpers + checkpoint helpers
add(code("""\
...
# CELL 6 – model save/load helpers
add(code("""\
...
# CELL 7 – Summary / plotting infrastructure
add(code("""\
...
# CELL 8 – Language config (Bengali-centric, NO Chinese)
add(md("## Language Configuration — Bengali-Centric (Ben, Eng, Hin, Arb)"))
...
# CELL 9 – ASR stack (MMS + Whisper, NO Chinese)
add(md("## ASR Stack — MMS (Ben/Hin/Arb) + Whisper (Eng)"))
...
# CELL 10 – sacrebleu metrics + inference helpers
add(code("""\
...
# CELL 11 – Dataset loading (parquet streaming, Bengali-centric)
add(md("## Dataset Loading — Bengali-Centric Streaming"))
...
# CELL 12 – Load eval and training datasets
add(code("""\
...
# CELL 13 – Model loading, sync, session status
add(code("""\
from transformers import SeamlessM4Tv2ForSpeechToSpeech, SeamlessM4TProcessor
...
# Phase 2 — Speech Encoder: 24→16 (first pass)
...
# Phase 3 — T2U LaCo (6→4 enc, 6→4 dec: prune 2 each)
...
# Phase 4 — Speech Encoder: 16→8 (second pass, total 16 removed)
...
# out_path = 'bengali_seamless_notebook.ipynb'
# with open(out_path, 'w', encoding='utf-8') as fh:
#     json.dump(NB, fh, ensure_ascii=False, indent=1)

# print(f'\\n✓ Generated notebook: {out_path}')
# print(f'  Cells   : {len(NB[\"cells\"])}')
# print(f'  Size    : {os.path.getsize(out_path)/1024:.1f} KB')
# print()
# print('Usage: python generate_bengali_notebook.py')
# print('Then upload bengali_seamless_notebook.ipynb to Kaggle.')
# # """)
